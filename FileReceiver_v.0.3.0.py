...
class MainWindow(wx.Frame):
    ...
    def __init__(self):
        try:
            ...
            wx.Frame.__init__(
                self, None, -1, "FileReceiver ver. " + MyDate,
                style = styleWindow)
...
#=========================================
    def ShowServerInfo(self):
        try:
            textDomain = "Your Domain name = " + str(socket.gethostbyaddr("localhost")[0])
            ToLog("Obtained Domain Name = " + str(socket.gethostbyaddr("localhost")[0]))
        except Exception as Err:
            textDomain = "Error obtain DomainName, try to find your IP by yourself"
        ...
        try:
            ...
            if len(tupIP) == 1:
                textIP = "Your IP = " + str(socket.gethostbyaddr("localhost")[0])
                textIP = str(socket.gethostbyname(socket.gethostname()))
            else:
                textIP = "Your PC has multiple IP-addresses: "
                for word in tupIP:
                    textIP = textIP + "\n" + word
            ...
            textIP = "Error obtain IP address, try to find your IP by yourself"
            ToLog("Error obtain IP address, Error code = " + str(Err))
            #raise Exception

        textPort = "Your Server Port = " + str(self.Settings[3]) + "\nTell this to Remote Client"
    ...
    def SendBtnFunc(self, evt):
        ...
        try:
            ...
            if self.TransvThread == None:
                ...
                wx.MessageBox("Transv thread is already running", " ", wx.OK)
                return
            ...
            wx.MessageBox("Error in ChooseFile, Error code = " + str(Err), " ", wx.OK)
            #raise Exception
            return
        ...
        try:
            ...
            wx.MessageBox("Error in Entering Server Parameters, Error code = " + str(Err), " ", wx.OK)
            #raise Exception
            return

        if self.CheckMd5Thread.is_alive() == True:
            result = self.WaitMd5(file)
            ToLog("RESULT OF WAITMD5 = " + result)
            if result == "False":
                wx.MessageBox("Error in finding Md5 hash of file " + file, " ", wx.OK)
                return
    ...
    def WaitMd5(self, file):
        try:
            ...
            WarnDlg = wx.GenericProgressDialog(
                "",
                "Wait for counting MD5 hash of " + file,
                maximum = 5,
                parent = self.frame,
    ...
    def EnterServerPams(self):
        ...
        if EnterDlg.ShowModal() == wx.ID_OK:
            if EnterDlg.EnteredValue[1].GetValue().isdigit():
                ...
                wx.MessageBox("Value of Port is not digit, Port Value set on 10002", " ", wx.OK)
    ...
    def UpdateDisplay(self, mess):
        try:
            ...
            if mess == "CloseDlg":
                ...
                self.OpenUpdDlg(
                    "Progress of receiving " + self.FileAskWin,
            ...
            if isinstance(mess, list):
                if mess[0] == "showWin":
                    ...
                    #self.ansdlg = dlg = wx.MessageDialog(self, mess[1], " ", wx.YES_NO)
                    #dlg.SetYesNoLabels("&Да", "&Нет")
                    #if dlg.ShowModal() != wx.ID_YES:
                    #    self.ReceiverThread.SetAskMainWin("deslined")
                    #else:
                    #    self.OpenUpdDlg(
                    #        "Progress of receiving " + mess[2],
                    #        mess[2], mess[3] + 2000)
                    #    self.ReceiverThread.SetAskMainWin("agreed")
                    ...
                    if self.UpdDlg.WasCancelled() == True:
                        ...
                        self.UpdDlg.Update(
                            mess[1],
                            "Progress of receiving " + mess[2] +
                            " (" +  str(mess[1]) + " of " + str(mess[3]) + ") bytes")                          
                elif mess[0] == "newPort":
                    ...
                    self.UpdDlg.Destroy()
                    wx.MessageBox(
                        "File with name = " + str(mess[1]) + ", size = " + str(mess[2]) +
                        " received successfully, find it in " + self.Settings[1],
                        " ", wx.OK)
                    global MyDir
                    os.startfile(self.Settings[1])
                elif mess[0] == "OpenTrDlg":
                    self.OpenUpdDlg(
                        "Progress of transceiving " + mess[1],
                        mess[1], mess[2] + 2000)
                    ...
                    if self.UpdDlg.WasCancelled() == True:
                        ...
                        self.UpdDlg.Update(
                            mess[1],
                            "Progress of transceiving " + mess[2] +
                            " (" +  str(mess[1]) + " of " + str(mess[3]) + ") bytes")
                elif mess[0] == "SendFileSuccess":
                    self.UpdDlg.Destroy()
                    wx.MessageBox(
                        "End of transieving File " + str(mess[1]) + ", size = " + str(mess[2]),
                        " ", wx.OK)
                elif mess[0] == "CheckRecMd5":
                    self.UpdDlg.Update(
                        self.UpdDlg.GetValue(),
                        "Wait for counting md5 hash of " + mess[1])
                elif mess[0] == "NotOkMd5":
                    self.UpdDlg.Destroy()
                    wx.MessageBox(
                        "Received file " + mess[1] + "md5 hash (" + mess[3] +
                        ") does not compare with control md5 hash (" + mess[2] + ")" +
                        "\nReceived file may be not OK, try to transcieve file again"," ", wx.OK)
                elif mess[0] == "OpenWaitWin":
                    self.WaitDlg = wx.GenericProgressDialog(
                        "",
                        "Waiting for answer of " + mess[2] + " on receiving " + mess[1] + " (" + str(mess[3]) + ") sec",
                        maximum = 1,
                        parent = self.frame,
                        style = wx.PD_AUTO_HIDE|wx.PD_APP_MODAL|wx.PD_SMOOTH|wx.PD_CAN_ABORT)
                elif mess[0] == "EditWaitWin":
                    if self.WaitDlg.Update(0, "Waiting for answer of " + mess[2] + " on receiving " + mess[1] + " (" + str(mess[3]) + ") sec")[0] == False:
                    ...
                    self.WaitDlg.Destroy()
                    wx.MessageBox(
                        mess[2] + " didn't answer on request of receiving file " + mess[1], " ", wx.OK)
                    self.TransvThread.OffThread()
                    self.TransvThread.join()
    ...
    def ToWin(self, message):
        ...
        try:
            ...
            if message[0] == "RecUpdate" or message[0] == "TrUpdate":
                ...
                ToLog("Message to MainWin = " + str(message))
        except Exception:
    ...
    def FindFreePort(self, port = 10002, end = 16000):
        while True:
            if port > end:
                ...
                self.ToWin("Didn't find free port between " + str(port)  + " and " + str(end))
                return False
        ...
        try:
            ...
            if currFiles != False:
                if "." in Name:
                    ...
                    main = Name[:Name.rfind(".")]
                    ToLog("main, ext = " + main  + ", " + ext)
                else:
                    ext = ""
                    main = Name
                    ToLog("main, ext = " + main  + ", " + ext)
    ...
    def AskForReceive(self, file, size):
        try:
            ...
            try:
                ...
                textDomain = " "
            self.ToWin(["AskWin", "User with IP " + self.addr[0] + textDomain + 
                        " trying to send file to your PC. \nProceed?" +
                        "\n\n\tname of file = " + file + ", \n\tsize " +
                        str(size) + " (in bytes)", file, size])
            while True:
...
class TransiverThread(MyThread):
    ...
    def TransvFile(self, buff_size = 4096, sep = "<SEP>"):
        ...
        with open(self.filepath, "rb") as File:
            ...
            #self.sock.send(f"{FileName}{sep}{FileSize}".encode("ISO-8859-1"))
            ...
            while True:
                ...
                if recv == "wait":
                    ...
                    WaitThread.GotAnswer()
                    self.ToWin(["showWin", "Transv file was cancelled by receiving side"])
